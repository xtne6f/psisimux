...
namespace
{
inline uint16_t &LittleToHostEndian(uint16_t &x)
{
#ifndef _WIN32
    int e = 1;
    if (!*reinterpret_cast<char *>(&e)) {
...
inline uint32_t &LittleToHostEndian(uint32_t &x)
{
#ifndef _WIN32
    int e = 1;
    if (!*reinterpret_cast<char *>(&e)) {
        x = static_cast<uint32_t>(x) << 24 | (x & 0xFF00) << 8 | (x & 0xFF0000) >> 8 | x >> 24;
    }
#endif
    return x;
}
...
#ifdef _WIN32
bool CPsiArchiveReader::Open(const wchar_t *path)
#else
bool CPsiArchiveReader::Open(const char *path)
#endif
{
    Close();
    m_chunkFileOffset = -1;
...
bool CPsiArchiveReader::ReadCodeList(const std::function<void(int, uint16_t, uint16_t)> &proc, const char *&errorMessage)
{
    ...
    for (;;) {
        ...
        for (size_t i = 0; i < dictionaryLen; ++i) {
            ...
            if (LittleToHostEndian(dict[i].first) >= CODE_NUMBER_BEGIN) {
                // 前回辞書を参照
        ...
        for (size_t i = 0; i < dictionaryLen; ++i) {
            if (dict[i].second == 0xFFFF) {
                ...
                LittleToHostEndian(dict[i].second) &= 0x1FFF;
            }
        }
        ...
        for (size_t i = 0; i < m_timeList.size(); ++i) {
            if (m_timeList[i] == UNKNOWN_TIME) {
                ...
                for (size_t n = (m_timeList[i] >> 16) + 1; n > 0; --n) {
                    ...
                    if (LittleToHostEndian(code) < CODE_NUMBER_BEGIN ||
        ...
        for (size_t i = m_timeListIndex; i < m_timeList.size(); ++i, ++m_timeListIndex) {
            if (m_timeList[i] == UNKNOWN_TIME) {
                ...
                for (size_t n = (m_timeList[i] >> 16) + 1; n > 0; --n) {
                    uint16_t code;
                    if (fread(&code, sizeof(uint16_t), 1, m_fp.get()) != 1 ||
                        LittleToHostEndian(code) < CODE_NUMBER_BEGIN ||
...
int64_t CPsiArchiveReader::MoveToNextChunk()
{
    ...
    for (size_t i = 0; i < dictionaryLen; ++i) {
        ...
        if (LittleToHostEndian(codeOrSize) >= CODE_NUMBER_BEGIN) {
            // 前回辞書を参照
    ...
    for (size_t i = 0; i < dictionaryLen; ++i) {
        if (m_dict[i].second == 0xFFFF) {
            ...
            dictionaryDataSize -= 2;
            LittleToHostEndian(m_dict[i].second) |= 0xE000;
        }
    }
    for (size_t i = 0; i < dictionaryLen; ++i) {
                                   ...
                                   uint32_t &dictionaryDataSize, uint32_t &dictionaryBuffSize, uint32_t &codeListLen)
{
    ...
    if (fread(&magicAndReserved, 1, 10, fp) != 10 ||
        ...
        LittleToHostEndian(dictionaryWindowLen) < LittleToHostEndian(dictionaryLen) ||
        LittleToHostEndian(dictionaryBuffSize) < LittleToHostEndian(dictionaryDataSize) ||
        dictionaryWindowLen > 65536 - CODE_NUMBER_BEGIN)
    {
        return false;
    }
    LittleToHostEndian(codeListLen);
    timeList.resize(LittleToHostEndian(timeListLen));
    if (!timeList.empty() && fread(timeList.data(), sizeof(uint32_t), timeList.size(), fp) != timeList.size()) {
        return false;
    }
    for (size_t i = 0; i < timeList.size(); ++i) {
        LittleToHostEndian(timeList[i]);
    }
    return true;
}
