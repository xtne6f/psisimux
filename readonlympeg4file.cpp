...
int CReadOnlyMpeg4File::Read(uint8_t *pBuf, int numToRead)
{
    if (m_fp) {
        ...
        while (numRead < numToRead) {
            ...
            int n = static_cast<int>(std::min<int64_t>(numToRead - numRead, static_cast<int64_t>(m_blockCache.size()) - m_pointer));
...
int64_t CReadOnlyMpeg4File::SetPointer(int64_t distanceToMove, MOVE_METHOD moveMethod)
{
    if (m_fp) {
        ...
        if (toMove >= 0) {
            BLOCK_100MSEC val;
            val.pos = static_cast<uint32_t>(std::min<int64_t>(toMove / 188, 0xFFFFFFFF));
...
void CReadOnlyMpeg4File::SetBroadcastID(uint16_t nid, uint16_t tsid, uint16_t sid)
{
    m_nid = nid;
    m_tsid = tsid;
    m_sid = sid;
...
void CReadOnlyMpeg4File::SetTotStartTime(uint32_t t)
{
    m_totStart = t;
}
        ...
        if (timeScale != 0) {
            ...
            if (m_stsoV.empty() && ReadVideoSampleDesc(trakBoxPos, m_fHevc, m_spsPps, buf)) {
                ...
                if (ReadSampleTable(trakBoxPos, m_stsoV, m_stszV, m_sttsV, &m_cttsV, editTimeOffset, buf) &&
                    ...
                    m_offsetPtsV = static_cast<uint32_t>(22500 + std::min<int64_t>(std::max<int64_t>(editTimeOffset * 45000 / m_timeScaleV, 0), 22500));
                }
                else {
                    m_stsoV.clear();
                }
                ...
                if (ReadSampleTable(trakBoxPos, m_stsoA[0], m_stszA[0], m_sttsA[0], nullptr, editTimeOffset, buf) &&
                    ...
                    m_offsetPtsA[0] = static_cast<uint32_t>(22500 + std::min<int64_t>(std::max<int64_t>(editTimeOffset * 45000 / m_timeScaleA[0], 0), 22500));
                }
                else {
                    m_stsoA[0].clear();
                }
                ...
                if (ReadSampleTable(trakBoxPos, m_stsoA[1], m_stszA[1], m_sttsA[1], nullptr, editTimeOffset, buf) &&
                    ...
                    m_offsetPtsA[1] = static_cast<uint32_t>(22500 + std::min<int64_t>(std::max<int64_t>(editTimeOffset * 45000 / m_timeScaleA[1], 0), 22500));
                }
                else {
                    m_stsoA[1].clear();
                }
            ...
            for (; buf.size() >= 8; buf.erase(buf.begin(), buf.begin() + boxLen)) {
                ...
                // Type == "hvcC"
                    ...
                    if (30 < buf.size()) {
                        ...
                        spsPps.clear();
                        bool fFoundAll = true;
                        for (size_t naluTypeIndex = 0; fFoundAll && naluTypeIndex < sizeof(NALU_TYPES) / sizeof(NALU_TYPES[0]); ++naluTypeIndex) {
                            fFoundAll = false;
                            size_t pos = 31;
...
bool CReadOnlyMpeg4File::ReadCurrentBlock()
{
    ...
    if (m_psiDataReader.IsOpen()) {
        ...
        if (blockIndex % 20 == 0) {
            ...
            packet[4] = 0;
            CreateTot(packet + 5, m_totStart + static_cast<uint32_t>(blockIndex / 10));
        }
    }
...
{
    if ((currentBoxPos >= 0 && my_fseek(m_fp.get(), currentBoxPos, SEEK_SET) != 0) ||
    ...
    while (fread(head, 1, 8, m_fp.get()) == 8) {
        ...
        if (memcmp(path, head + 4, 4) == 0 && --index < 0) {
            if (path[5] == '\0') {
                return std::pair<int64_t, int64_t>(my_ftell(m_fp.get()), boxSize - numRead);
            }
            return FindBoxPosition(path + 6, -1);
        }
        if (my_fseek(m_fp.get(), boxSize - numRead, SEEK_CUR) != 0) {
            break;
        }
    }
    return std::pair<int64_t, int64_t>(-1, 0);
...
void CReadOnlyMpeg4File::AddTsPacketsFromPsi(std::vector<uint8_t> &buf, const uint8_t *psi, size_t psiSize, uint8_t &counter, uint16_t pid)
{
    for (size_t i = 0; i < psiSize; ) {
        ...
        buf.insert(buf.end(), psi + i, psi + std::min(i + 184 - pointer, psiSize));
                                              ...
                                              bool fHevc, bool fAudio2, bool fCaption)
{
    ...
    if (static_cast<size_t>(12 + programInfoLength) <= pmt.size() - 4) {
        data[11] = CreatePmtProgramInfoFromProgramInfo(data + 12, &pmt[12], programInfoLength);
    }
    size_t x = 12 + data[11];
    x += CreatePmt2ndLoop(data + x, fHevc, fAudio2, fCaption);
...
uint8_t CReadOnlyMpeg4File::CreatePmtProgramInfoFromProgramInfo(uint8_t *data, const uint8_t *programInfo, uint16_t programInfoLength)
{
    uint8_t x = 0;
    for (int i = 0; i + 3 < programInfoLength; i += 2 + programInfo[i + 1]) {
        // ARIB限定受信方式の限定受信方式記述子かアクセス制御記述子があれば
        if ((programInfo[i] == 0x09 || programInfo[i] == 0xF6) &&
            programInfo[i + 1] >= 2 && programInfo[i + 2] == 0x00 && programInfo[i + 3] == 0x05)
        {
            // 一部の再生ソフトはこれらの記述子の有無でTSの規格を推測しているため
            data[x++] = 0x09; // 限定受信方式記述子
            data[x++] = 4;
            data[x++] = 0x00;
            data[x++] = 0x05;
            data[x++] = 0xFF; // PID=0x1FFF
            data[x++] = 0xFF;
            data[x++] = 0xC1; // デジタルコピー制御記述子
            data[x++] = 1;
            data[x++] = 0x04;
            data[x++] = 0xF6; // アクセス制御記述子
            data[x++] = 4;
            data[x++] = 0x00;
            data[x++] = 0x05;
            data[x++] = 0xFF; // PID=0x1FFF
